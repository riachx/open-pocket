...
app.get('/api/politician/:lastName/money-report', async (req, res) => {
  ...
  try {
    ...
    const moneyReport = await runPythonFunction('generate_comprehensive_money_report', [candidateId]);
...
app.get('/api/politician/:lastName/committees', async (req, res) => {
  ...
  try {
    ...
    console.log('Fetching PACs for candidate ID:', candidateId);
    // Get PACs by type from Python function
    const pacsByType = await runPythonFunction('get_politician_pacs', [candidateId]);
    console.log('Received PACs data:', JSON.stringify(pacsByType, null, 2));

    console.log('Fetching contribution totals for candidate ID:', candidateId);
    // Get contribution totals
    const contributionTotals = await runPythonFunction('get_politician_total_contributions', [candidateId]);
    console.log('Received contribution totals:', JSON.stringify(contributionTotals, null, 2));
...
app.get('/api/politician/:lastName/industries', async (req, res) => {
  ...
  try {
    ...
    const corporateConnections = await runPythonFunction('get_politician_corporate_connections', [candidateId]);
...
app.get('/api/senator/:id/money-report', async (req, res) => {
  ...
  try {
    ...
    console.log('Looking for candidate ID for senator:', senatorName);
    ...
    if (candidateMatch) {
      ...
      console.log('Found exact match for candidate ID:', candidateId);
    } else {
      // Try partial match with last name
      const lastName = senatorName.split(',')[0] || senatorName.split(' ').pop();
      console.log('Trying partial match with last name:', lastName);
      ...
      if (partialMatch) {
        ...
        console.log('Found partial match for candidate ID:', candidateId);
      } else {
    ...
    if (!candidateId) {
      console.log('No candidate ID found, returning empty data');
      return res.json({
        totalContributions: 0,
        totalCommittees: 0,
        committees: [],
